...
Использую для того, чтобы повесить iSpy на домен и закрыть за Basic Auth авторизацией (безопасность). P.S. Пароль на анель вроде можно указать в `Server (сверху слева) > Settings > Local Settings (сверху справа модалки) > Username, Password`, но я не пробовал.
...
1. Не уверен, что переключатель Enabled влияет на запуск Timelapse по расписанию (Schedule). Но включение тут мгновенно запускает его генерацию
